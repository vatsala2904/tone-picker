...
// Rate limit (don’t block OPTIONS)
...
function sanitize(s){ return String(s || '').slice(0, 8000) }
...
app.post('/api/tone', async (req,res)=>{
  try{
    ...
    if (!r.ok){
      ...
      return res.status(502).json({ message:`Upstream error ${r.status}: ${errTxt.slice(0,200)}` })
    }
    ...
    cset(cacheKey, output, 5 * 60_000) // 5 min
    res.json({ output })
  }catch(e){
    console.error(e)
